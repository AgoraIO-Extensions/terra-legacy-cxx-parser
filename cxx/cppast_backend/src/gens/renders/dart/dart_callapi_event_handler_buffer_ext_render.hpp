#ifndef DART_CALLAPI_EVENT_HANDLER_BUFFER_EXT_RENDER_H_
#define DART_CALLAPI_EVENT_HANDLER_BUFFER_EXT_RENDER_H_
...
class DartCallApiEventHandlerBufferExtRender : public DartSyntaxRender
{
private:
    std::string file_name_ = "call_api_event_handler_buffer_ext.dart";
...
public:
    ...
    //     // Struct s;
    //     // if (FindStruct(member_variable.type.name, s) == 0)
    //     // {
    //     //     actual_type += "Json";
    //     // }
    ...
    //     if (std::find(IgnoreJsonType.begin(), IgnoreJsonType.end(), actual_type) != IgnoreJsonType.end())
    //     {
    ...
    RenderedBlock RenderStruct(const Struct &original_struct, const std::vector<RenderedBlock> &rendered_struct_members) override
    {
        ...
        dart_class += RenderCallApiEventHandlerBufferExt(struct_name, original_struct.member_variables).rendered_content;
...
#endif // DART_CALLAPI_EVENT_HANDLER_BUFFER_EXT_RENDER_H_